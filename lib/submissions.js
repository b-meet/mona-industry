...
function describe(error, action) {
    if (error.code === '42P01') {
        return `${action} failed: the "inquiries" table does not exist. Run supabase/migrations/20260919120000_init_submissions.sql against the project.`;
    }
    if (error.code === '42501' || /row-level security/i.test(error.message || '')) {
        return `${action} failed: row level security rejected the request. Check the policies in supabase/migrations/20260919120000_init_submissions.sql.`;
    }
