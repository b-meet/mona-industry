...
/**
 * Turns a failed response into an Error carrying the server's message plus the
 * status and, on a 500, the one-word cause code the route attaches. Callers
 * use `status` to decide whether the visitor can act on the message: a 400
 * names something they typed, a 500 is ours to fix.
 */
...
export async function createSubmission({
    ...
    if (!response.ok) throw await readError(response, 'Submitting your enquiry failed');
}
...
export async function listSubmissions() {
    ...
    if (!response.ok) throw await readError(response, 'Loading submissions failed');
...
export async function updateSubmissionStatus(id, status) {
    ...
    if (response.status === 401) throw new Error('NOT_AUTHENTICATED');
    if (!response.ok) throw await readError(response, 'Updating the status failed');
}
