...
/** Fail before the network call when the build shipped without credentials. */
function assertConfigured() {
    if (!isSupabaseConfigured) throw new Error(SUPABASE_NOT_CONFIGURED);
}

/**
 * Turns a PostgREST error into something a developer can act on. The visitor
 * only ever sees the form's own friendly line; this is what lands in the
 * console and in any error report.
 */
function describe(error, action) {
    const MIGRATION = 'supabase/migrations/20260919120000_init_submissions.sql';

    // PGRST205: PostgREST answers 404 because "inquiries" is missing from its
    // schema cache. Either the table was never created, or it was created by
    // SQL after PostgREST last read the schema.
    if (error.code === 'PGRST205') {
        return (
            `${action} failed: the Data API does not know the "inquiries" table (404). ` +
            `Run \`notify pgrst, 'reload schema';\` in the SQL Editor, confirm Settings -> Data API ` +
            `exposes the public schema, and check NEXT_PUBLIC_SUPABASE_URL points at the project ` +
            `where ${MIGRATION} was run.`
        );
    }
    if (error.code === '42P01') {
        return `${action} failed: the "inquiries" table does not exist. Run ${MIGRATION} against the project.`;
    }
    if (error.code === 'PGRST204') {
        return `${action} failed: ${error.message}. The table is missing a column the site writes — re-run ${MIGRATION}.`;
    }
    if (error.code === '42501' || /row-level security/i.test(error.message || '')) {
        return `${action} failed: row level security rejected the request. Check the policies in ${MIGRATION}.`;
    }
    if (error.code === '23514') {
        return `${action} failed: a check constraint rejected the row (${error.message}). Most likely an invalid email or an over-long field.`;
    }
    if (error.code === 'PGRST301' || error.code === '401') {
        return `${action} failed: the anon key was rejected. Check NEXT_PUBLIC_SUPABASE_ANON_KEY belongs to the same project as NEXT_PUBLIC_SUPABASE_URL.`;
    }
...
export async function createSubmission({
    ...
    if (error) throw new Error(describe(error, 'Submitting your enquiry'));
}
...
export async function listSubmissions() {
    ...
    if (error) throw new Error(describe(error, 'Loading submissions'));
    return data || [];
}
...
export async function updateSubmissionStatus(id, status) {
    ...
    if (error) throw new Error(describe(error, 'Updating the status'));
}
